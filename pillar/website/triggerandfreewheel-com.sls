#!yaml|gpg

website:
  triggerandfreewheel-com:
    enable_ssl: True

    # NOTE: Be very careful if generating this via the shell. There is usually a
    # $ symbol in here that the shell likes to eat. Always verify that the
    # managed file contains all the expected data after modifications here.
    user_list: |
      -----BEGIN PGP MESSAGE-----

      
      =3Fvw
      -----END PGP MESSAGE-----
